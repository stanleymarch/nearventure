...
$COMPOSE --profile import run --rm poi-importer --dry-run --run-dir releases/2026-07-26
...
$COMPOSE --profile import run --rm poi-importer --run-dir releases/2026-07-26
```

Локальная разработка (из `apps/backend`, без сборки):

```bash
npm run import:poi -- --trusted-root /abs/path/to/imports --run-dir releases/2026-07-26
```
